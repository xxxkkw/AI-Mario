...
class Mario(pg.sprite.Sprite):
    ...
    def standing(self, keys, fire_group):
        ...
        if config.mode == "player":
            ...
            if keys[tools.key_map['action']]:
                if self.fire and self.allow_fireball:
                    self.shoot_fireball(fire_group)

            if keys[tools.key_map['down']]:
                self.crouching = True

            if keys[tools.key_map['left']]:
                self.facing_right = False
                self.get_out_of_crouch()
                self.state = c.WALK
            elif keys[tools.key_map['right']]:
                self.facing_right = True
                self.get_out_of_crouch()
                self.state = c.WALK
            elif keys[tools.key_map['jump']]:
                if self.allow_jump:
                    if self.big:
                        setup.SFX['big_jump'].play()
                    else:
            ...
            if not keys[tools.key_map['down']]:
                self.get_out_of_crouch()
    ...
    def check_to_allow_jump(self, keys):
        ...
        if config.mode == "player":
            ...
            if not keys[tools.key_map['jump']]:
                self.allow_jump = True
    ...
    def check_to_allow_fireball(self, keys):
        ...
        if config.mode == "player":
            ...
            if not keys[tools.key_map['action']]:
                self.allow_fireball = True
